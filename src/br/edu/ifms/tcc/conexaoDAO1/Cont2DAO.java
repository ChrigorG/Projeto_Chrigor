 ...
 * @author Noah
 */
public class Cont2DAO{
    private static int numquest = 1;
    private static int idquest = 0;
    private static int max = 29;
    static Usuario usuario = new Usuario();
    public static void receberquestao(Conteudo2 rq){
        idquest = idquest + 1;
        String sql = "SELECT * FROM Cont2 WHERE id2_questao =" + idquest;
        try{
        if(max >= idquest){
        PreparedStatement pstm = new Conexao().fazerConexao().prepareStatement(sql);
        ResultSet rs = pstm.executeQuery();
        while(rs.next()){
            rq.setExercicio(rs.getString("exercicio2"));
            rq.setAlt1(rs.getString("alternativa2_1"));
            rq.setAlt2(rs.getString("alternativa2_2"));
            rq.setAlt3(rs.getString("alternativa2_3"));
            rq.setAlt4(rs.getString("alternativa2_4"));
            rq.setAlt5(rs.getString("alternativa2_5"));
            rq.setResult(rs.getString("resultado2"));
            numquest = idquest + 31;
    ...
    public static void Contador(Conteudo2 rq) {
        int maxace;
        ...
        ResultSet rs = pstm.executeQuery();
            while(rs.next()){
                ...
                if (rq.getResult() == "correta"){
                    contador = rs.getInt("n2_exe_aluno") + 1;
                    System.out.println("acertou!");
                }
                String sql1 = "UPDATE Aluno SET n2_exe_aluno = "+contador+" WHERE nome_aluno = ?";
                try{
        ...
        if(max == idquest){
                    ...
                    ResultSet rs3 = pstm3.executeQuery();
                        while(rs3.next()){
                            idquest = -1;
                            rq.setProgresso(rs3.getInt("n2_exe_aluno"));
                            if(rs3.getInt("n2_exe_aluno") > rs3.getInt("max_n2")){
                            maxace = rs3.getInt("n2_exe_aluno");
                            String sql2 = "UPDATE Aluno SET max_n2 = "+maxace+" WHERE nome_aluno = ?";
     ...
     public static void Historico(Conteudo2 rq) throws SQLException{
        String sql2 = "INSERT INTO HistoricoPontuacao (pont2,questoes_erradas,nome_aluno) VALUES (?,?,?);";
                            PreparedStatement pstm2 = new Conexao().fazerConexao().prepareStatement(sql2);
